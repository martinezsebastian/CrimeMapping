...
**Table 1** shows the types of crimes included in the City of Chicago dataset with their respective number of reported instances for the year 2012. The data presents all districts and all crime types. 

```{r kable Table 1, echo=FALSE}
kable(as.matrix(table1), digits = 2, caption = "Number of crimes committed in every District accross all types") 
```

**Table 2** presents the *relative* distance between crime scene and Police Station as well as to the police district center. The table again shows all crime types and presents the average across all districts. 

```{r kable Table 2, echo=FALSE}
kable(as.matrix(table2), digits = 2, caption = "Relative Distance to Policy Station and to District Center accross all crime types") 

```


As shown in **Table 2**, the distances are not the same, but lead to overall similar results. Differences occur because the police station is not always located at the center of the district, as it is calculated in this analysis. 
...
However, this aggregated level clearly does not allow for much variation in the relative distances. Most of the observations are clustered around very similar values. A closer look at more disaggregated data seems to be necessary to discover spatial patterns of the perpetration of crime on the district level. 
...
But first, we need to test wheter our caluclation of *relative* distances of certain crimes to the crime farthest away per district adds value to our statistical analysis. 
...
## EXCLUDE INtimidation (!) and sex offense  (!)
