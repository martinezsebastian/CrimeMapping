...
library(knitr)
kable(as.matrix(agg_crime), digits = 2, caption = "Aggregate Crime Statistics") 
...
As we can see in Figure 4, looking at the data for each district separately leads to improvements but still yields only slightly better results. 
...
Given these results, we are rethinking the use of the relative distance calculation. The absolute distance, of course, provides more diverse results.  
